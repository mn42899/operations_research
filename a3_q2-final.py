...
# -----------------------------
# Solve the Mean Value Problem (EV Solution)
# -----------------------------
def solve_mean_value_model():
    ...
    m.optimize()
    return truck_size.X  # We return only the truck size

# -----------------------------
# Evaluate the EV Solution in Stochastic Scenarios (EEV)
# -----------------------------
def evaluate_eev(truck_size_val, trials=20, scenarios_per_trial=10, seed=2025):
    rng = np.random.default_rng(seed)
    trial_costs = []
    ...
    for t in range(trials):
        ...
        for _ in range(scenarios_per_trial):
            ...
            u = m.addVars(active, lb=1, ub=len(active))

            total_demand = sum(demands[s] for s in active)

            m.addConstr(over >= truck_size_val - total_demand)
            m.addConstr(under >= total_demand - truck_size_val)
            ...
            travel = gp.quicksum(cost_matrix[i, j] * x[i, j] for i, j in arcs)
            penalty = 0.09 * over + 0.13 * under
            m.setObjective(travel + penalty, GRB.MINIMIZE)
            m.optimize()
            scen_costs.append(m.objVal)
    ...
    print(f"\nEEV (Expected Value using deterministic solution): ${eev:.2f}")
    return eev
...
# -----------------------------
if __name__ == "__main__":
    saa_cost = 158.94  # Replace with your actual SAA cost from part (e)
    ev_truck_size = solve_mean_value_model()
    eev = evaluate_eev(ev_truck_size)
    vss = eev - saa_cost
